...
class CSI_D_base(Dataset):
    ...
    def __prepare(self):
        ...
        for stock in tqdm(stocks_list):
            ...
            self.item_list += [f"{self.root_path}/{stock}"]*self.repeat
            self.label_list += [label_id]*self.repeat
            label_id +=1
    ...
    def __getitem__(self, i):
        ...
        volume_p = df['vol'].to_numpy()
        turnover_p = df['amount'].to_numpy()
        change_p = df['change'].to_numpy()



        if self.segment_length>0:
            start_idx = np.random.randint(low=self.start_idx, high=min(self.end_idx,open_p.shape[0])-self.segment_length)
            ...
            start_idx = min(self.start_idx,open_p.shape[0]-self.end_idx+self.start_idx)
            end_idx = min(self.end_idx,open_p.shape[0])
            open_p = open_p[start_idx:end_idx]
            close_p = close_p[start_idx:end_idx]
            high_p = high_p[start_idx:end_idx]
            low_p = low_p[start_idx:end_idx]

            volume_p = volume_p[start_idx:end_idx]
            turnover_p = turnover_p[start_idx:end_idx]
            change_p = change_p[start_idx:end_idx]
