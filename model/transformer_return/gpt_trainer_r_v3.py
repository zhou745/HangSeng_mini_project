...
class GPT_stock_r(pl.LightningModule):
    ...
    def training_step(self, batch,batch_idx):
        ...
        preditions_prev = preditions[:,:self.obeserve_length,0]
        target_prev = feature_train[:,:,7]
        ...
        loss_dir = 1-diff_mean/std_prod
        loss_dir = loss_dir.mean()
        #loss for amplitude
        # loss_amp = torch.abs(loss_dir-target).mean()
        loss_amp_prev =torch.abs(preditions_prev-target_prev).mean()
        loss = loss_dir+loss_amp_prev*0.1
