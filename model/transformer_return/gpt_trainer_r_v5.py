...
class GPT_stock_r(pl.LightningModule):
    ...
    def training_step(self, batch,batch_idx):
        ...
        # loss_dir = torch.nn.functional.binary_cross_entropy(torch.sigmoid(dir_pred),label.view(-1,1))
        #use focal loss
        gamma = 0.75
        p = torch.sigmoid(dir_pred)
        ce_loss = F.binary_cross_entropy_with_logits(dir_pred, label.view(-1,1), reduction="none")
        p_t = p * label.view(-1,1) + (1 - p) * (1 - label.view(-1,1))
