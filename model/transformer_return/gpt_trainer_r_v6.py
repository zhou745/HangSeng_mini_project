...
class GPT_stock_r(pl.LightningModule):
    ...
    def training_step(self, batch,batch_idx):
        ...
        label = (target>0.).to(torch.float32)
        #we use a second label
        label_trend = (feature_train[:,:,-1].sum(dim=-1)>self.trend_thr).to(torch.float32)

        label_dir = (label.view(-1,1)+label_trend.view(-1,1))/2
        ...
        # render with gsp
        # dir_pred = dir_pred.contiguous().view(B,(N-self.obeserve_length+1))
        #
        # diff_mean = (dir_pred*target).mean(dim=1)-dir_pred.mean(dim=1)*target.mean(dim=1)
        # std_prod = ((dir_pred.std(dim=1)+1e-6)*(target.std(dim=1)+1e-6))
        # #
        # loss_ic = 1-diff_mean/std_prod
