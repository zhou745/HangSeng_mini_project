...
class GPT_stock_r(pl.LightningModule):
    ...
    def training_step(self, batch,batch_idx):
        ...
        # print([batch['open'][i].dtype for i in range(len(batch['open']))])
        #
        # # print(type(batch['close']))
        # # print(type(batch['high']))
        # # print(type(batch['low']))
        # # print(type(batch['volume']))
        # # print(type(batch['turnover']))
        # # print(type(batch['change_rate']))
        # a = input()
        feature = torch.stack(feature_list, -1)

        pred_label = self.pred_label.to(feature.device)
        #we use a 24 tokens to 12 tokens prediction
        feature_train = feature[:, :-1].clone()

        B,N,_ = feature_train.shape
        feature_r_train = (feature_train[:,:,1]-feature_train[:,:,0])/feature_train[:,:,0]
        feature_r_train = feature_r_train.view(B,N,1).clamp(max=0.008,min=-0.008)
        ...
        for i in range(N-self.obeserve_length+1):
            ...
            feature_r_list.append(feature_r_train[:,i:i+self.obeserve_length])

        feature_r_train = torch.stack(feature_r_list, dim=1).contiguous()
        feature_r_train = feature_r_train.view(B*(N-self.obeserve_length+1),self.obeserve_length,-1)
        ...
        open_target = (feature_pred[:,:,0]-mean_price[:,None])/std_price[:,None]
        close_target = (feature_pred[:,:,1]-mean_price[:,None])/std_price[:,None]
