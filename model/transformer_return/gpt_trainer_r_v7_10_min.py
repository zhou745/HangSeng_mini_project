...
class GPT_stock_r(pl.LightningModule):
    ...
    def training_step(self, batch,batch_idx):
        ...
        feature_r_10_min = (feature_train[:,self.interval_num-1:,1]-feature_train[:,:-self.interval_num+1,0])/feature_train[:,:-self.interval_num+1,0]
        #normalize the data
        ...
        target = (feature_pred[:,self.interval_num-1:,1]-feature_pred[:,:-self.interval_num+1,0])/feature_pred[:,:-self.interval_num+1,0]
        ...
        op_pred,cl_pred,amp_pred = self(feature_train,stock_id,pred_label,feature_r_train)

        open_target = (feature_pred[:,:-self.interval_num+1,0]-mean_price[:,None])/std_price[:,None]
        close_target = (feature_pred[:,self.interval_num-1:,1]-mean_price[:,None])/std_price[:,None]

        loss_op = torch.abs(op_pred-open_target.view(-1,1)).mean()
        loss_cl = torch.abs(cl_pred-close_target.view(-1,1)).mean()
        #use focal loss
        # gamma = 0.75
        # p = torch.sigmoid(dir_pred)
        # ce_loss = F.binary_cross_entropy_with_logits(dir_pred, label_dir, reduction="none")
        # p_t = p * label_dir + (1 - p) * (1 - label_dir)
        # loss_dir = ce_loss * ((1 - p_t) ** gamma)
        # loss_dir = loss_dir.mean()*2
        loss_amp = torch.abs(amp_pred-torch.abs(target.view(-1,1)).clamp(max=0.008)).mean()

        # render with gsp
        loss = loss_amp+loss_op+loss_cl
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(0,feature.shape[1]-self.obeserve_length,self.test_step):
            ...
            feature_r_10_min = (feature_train[:, self.interval_num - 1:, 1] - feature_train[:, :-self.interval_num + 1,
                                                                              0]) / feature_train[:,
                                                                                    :-self.interval_num + 1, 0]
            ...
            #debug
            op_pred,cl_pred,amp_pred = self(feature_train, stock_id,pred_label,feature_r_10_min)
            #compute return correlation  #for now use this approximate formula
            dir_pred = cl_pred-op_pred
            return_actual = (feature_pred[:,self.interval_num-1:,1]-feature_pred[:,:-self.interval_num+1,0])/feature_pred[:,:-self.interval_num+1,0]
