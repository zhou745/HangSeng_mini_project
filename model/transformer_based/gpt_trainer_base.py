...
class GPT_stock_base(pl.LightningModule):
    def __init__(self,
        ...
        self.dummy_token = nn.Parameter(torch.rand((1,prediction_length,feature_dim),dtype=torch.float32))
    ...
    def forward(self, x,stock_id):
        ...
        predictions = self.network(x,stock_id)
        return predictions
    ...
    def training_step(self, batch,batch_idx):
        ...
        feature_train = torch.cat([feature_train,self.dummy_token.repeat(B,1,1)], dim=1)

        feature_pred = feature[:, self.obeserve_length:]
        feature_pred = feature_pred[:,:,2:4] #only predict first 2 values
        stock_id = batch['stock_id']
        # autoencode
        preditions = self(feature_train,stock_id)
    ...
    def configure_optimizers(self):
        ...
        l = [p for p in self.network.parameters()]+[self.dummy_token]
