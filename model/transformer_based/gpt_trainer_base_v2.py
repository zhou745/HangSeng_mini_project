...
class GPT_stock_base(pl.LightningModule):
    def __init__(self,
        ...
        self.pred_label = torch.tensor([[i for i in range(1)]],dtype=torch.long)
    ...
    def training_step(self, batch,batch_idx):
        ...
        target_price = torch.stack([max_price,min_price],dim=-1).view(B,1,-1)
        loss = torch.abs(preditions-(target_price-mean_price[:,None,None])/std_price[:,None,None]).mean()
        # print(preditions)

        # print((target_price-mean_price[:,None,None])/std_price[:,None,None])
        # print(preditions)
        # a = input()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(0,feature.shape[1]-self.obeserve_length,self.test_step):
            ...
            # print(target_price)
            # print(preditions)
            # print(list_for_auroc)
            # print(list_for_return_act)
            # print(list_for_return_pre)
        ...
        # render with gsp
        loss = torch.abs((preditions - target_price)*std_price).mean()
