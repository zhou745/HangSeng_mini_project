...
class Linear_stock_r(pl.LightningModule):
    def __init__(self,
                 moduleconfig,
                 feature_dim,
                 obeserve_length,
    ...
    def training_step(self, batch,batch_idx):
        ...
        feature_r_train = (feature_train[:,self.obeserve_length-1,1]-feature_train[:,self.obeserve_length-1,0])/feature_train[:,self.obeserve_length-1,0]
        feature_r_train = feature_r_train.clamp(max=0.02,min=-0.02).view(-1,1)

        feature_r_train_prev = (feature_train[:,self.obeserve_length-2,1]-feature_train[:,self.obeserve_length-2,0])/feature_train[:,self.obeserve_length-2,0]
        feature_r_train_prev = feature_r_train_prev.clamp(max=0.02, min=-0.02).view(-1,1)

        #compute ema
        feature_ema = feature_train[:,0]
        for i in range(self.obeserve_length-1):
            feature_ema = feature_ema*0.95+0.05*feature_train[:,i+1]

        #target to predict
        feature_r_10_min = (feature[:,-1,1]-feature[:,-self.interval_num,0])/feature[:,-self.interval_num,0]
        #normalize the data
        ...
        feature_cur = feature_train[:,-1].contiguous()
        feature_cur[:,:4] = (feature_cur[:,:4]-mean_price[:,None])/std_price[:,None]
        feature_cur[:, 4:6] = torch.log(feature_cur[:, 4:6]+base_offset)/10   #10k as unit
        feature_ema[:,  :4] = (feature_ema[:, :4] - mean_price[:, None]) / std_price[:, None]
        feature_ema[:, 4:6] = torch.log(feature_ema[:, 4:6] + base_offset) / 10  # 10k as unit

        feature_input = torch.cat([feature_cur,feature_ema,feature_r_train,feature_r_train_prev],dim=-1)
        #now we regroup feature train
        target = feature_r_10_min.contiguous().view(-1,1)
        ...
        amp_pred = self(feature_input)

        loss_amp = ((amp_pred-target)*100)**2
        loss_amp = loss_amp.mean()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(0,feature.shape[1]-self.obeserve_length,self.test_step):
            ...
            B, N, _ = feature_train.shape
            feature_r_train = (feature_train[:, self.obeserve_length - 1, 1] - feature_train[:,
                                                                               self.obeserve_length - 1,
                                                                               0]) / feature_train[:,
                                                                                     self.obeserve_length - 1, 0]
            feature_r_train = feature_r_train.clamp(max=0.02, min=-0.02).view(-1, 1)

            feature_r_train_prev = (feature_train[:, self.obeserve_length - 2, 1] - feature_train[:,
                                                                                    self.obeserve_length - 2,
                                                                                    0]) / feature_train[:,
                                                                                          self.obeserve_length - 2, 0]
            feature_r_train_prev = feature_r_train_prev.clamp(max=0.02, min=-0.02).view(-1, 1)

            # compute ema
            feature_ema = feature_train[:, 0]
            for j in range(self.obeserve_length - 1):
                feature_ema = feature_ema * 0.95 + 0.05 * feature_train[:, j + 1]

            # target to predict
            feature_r_10_min = (feature[:, self.obeserve_length+i+self.interval_num-1, 1] - feature[:, self.obeserve_length+i, 0]) / feature[:,
                                                                                                     self.obeserve_length+i,
                                                                                                     0]
            # normalize the data
            mean_price = feature_train[:, :, :4].contiguous().view(feature_train.shape[0], -1).mean(1)
            std_price = feature_train[:, :, :4].contiguous().view(feature_train.shape[0], -1).std(
                1) + 1e-3  # ensure no nan
            ...
            return_actual = feature_r_10_min.view(-1,1)
        ...
        # render with gsp
        loss = torch.abs(pred_r-act_r).mean()
    ...
    def compute_statistics(self,batch):
        ...
        for i in range(0, feature.shape[1] - self.obeserve_length, self.test_step):
            ...
            # target to predict
            feature_r_10_min = (feature[:, self.obeserve_length + i + self.interval_num - 1, 1] - feature[:,
                                                                                                  self.obeserve_length + i,
                                                                                                  0]) / feature[
                                                                                                        :,
                                                                                                        self.obeserve_length + i,
                                                                                                        0]
            # print(feature[:, self.obeserve_length + i + self.interval_num - 1, 1],feature[:, self.obeserve_length + i , 0])
            # normalize the data
            mean_price = feature_train[:, :, :4].contiguous().view(feature_train.shape[0], -1).mean(1)
            std_price = feature_train[:, :, :4].contiguous().view(feature_train.shape[0], -1).std(
                1) + 1e-3  # ensure no nan
            ...
            return_actual = feature_r_10_min
            return_pred = amp_pred
            list_for_return_act.append(return_actual)
            list_for_return_pre.append(return_pred)
            list_for_return_amp.append(amp_pred)
