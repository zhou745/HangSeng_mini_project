...
class GPT_stock_r(pl.LightningModule):
    ...
    def training_step(self, batch,batch_idx):
        ...
        amp_pred = self(feature_train,stock_id,pred_label)

        open_target = (feature_pred[:,:N_pre-self.interval_num+1,0]-mean_price[:,None])/std_price[:,None]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # render with gsp
        loss = torch.abs((amp_pred - (feature_pred[:,:,1]-feature_pred[:,:,0])/feature_pred[:,:,0])).mean()
